...
Em Python, dividmos os métodos, em 2 grupos: métodos de instância e métodos
de classe.
...
ATENÇÃO! Por mais que possamos criar nossas próṕrias funções utilizando dunder
...
Métodos são escritos em letras minúsculas. Se o nome for composto, o nome terá
...
print(f'Senha User 1: {user1._Usuario__senha}')  # Acesso de forma errada
print(f'Senha User 2: {user2._Usuario__senha}')  # Acesso de forma errada
...
print(Usuario.conta_usuarios())  # Forma correta
print(user.conta_usuarios())  # Possível, mas incorreto
...
class Usuario:
    ...
    @classmethod
    def conta_usuarios(cls):  # cls -> própria classe
        return f'Temos {cls.contador} usuário(s) no sistema'
